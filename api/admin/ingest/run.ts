...
export default async function handler(req: IncomingMessage, res: ServerResponse) {
  ...
  // Concurrency guard — claim the run row atomically (the partial unique index
  // ingest_runs_one_running_idx allows at most one 'running' row, so two
  // simultaneous POSTs can never both claim).
